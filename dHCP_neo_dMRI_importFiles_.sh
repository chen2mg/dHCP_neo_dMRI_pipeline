...
b0_A_P=$7          # phase encoding A to P direction
...
echo ''
echo Separate b0 volumes according to their PE direction and obtain
echo bvals and bvecs files. Round bvals to identify shells.
echo ============================================================================
idxLR=0
idxRL=0
idxAP=0
...
echo ''
echo Write bvecs, bvals and eddy acquisition parameters file.
echo ============================================================================
...
echo ''
echo "Reorient raw data and bvecs to to match the approximate orientation"
echo "of the standard template images (MNI152)"
...
if [ -e ${b0_A_P} ]; then
  echo ''
  echo "Option A: the A_to_P b0 is acquired, then apply A_to_P P_to_A to topup"
  echo ============================================================================
  read -p "Press enter to continue"
#  ${scriptsFolder}/utils/pickBestB0s ${rawFolder}/tmpData/data ${rawFolder}/tmpData/bvals ${rawFolder}/tmpData/eddyIndex.txt ${rotime[0]} ${noB0s} ${prepFolder}/topup
  ${FSLDIR}/bin/fslroi $b0_A_P ${rawFolder}/tmpData/A2P 0 1
  ${FSLDIR}/bin/fslroi ${rawFolder}/tmpData/data ${rawFolder}/tmpData/P2A 0 1
  ${FSLDIR}/bin/fslmerge -t ${prepFolder}/topup/phase ${rawFolder}/tmpData/A2P ${rawFolder}/tmpData/P2A

else
  unique_pedirs=(`cat ${rawFolder}/pedirs`)
  if [ `echo ${#unique_pedirs[@]}` -gt 1 ]; then
      echo ''
      echo "Option B: More than 1 phase encoding direction detected. Selecting best b0 volumes for topup"
      echo ============================================================================
      ${scriptsFolder}/utils/pickBestB0s ${rawFolder}/tmpData/data ${rawFolder}/tmpData/bvals ${rawFolder}/tmpData/eddyIndex.txt ${rotime[0]} ${noB0s} ${prepFolder}/topup
  else
      echo "Option C: 1 phase encoding direction detected. Setting first b0 as reference volume"
      echo "${first_b0}" > ${prepFolder}/topup/ref_b0.txt
  fi

  echo ''
  echo Sort raw data based on the selected reference volume
  echo ============================================================================
  read -p "Press enter to continue"
...
echo "copy files"
read -p "Press enter to continue"
#cp ${rawFolder}/tmpData/bvals ${rawFolder}/bvals
#cp ${rawFolder}/tmpData/bvecs ${rawFolder}/bvecs
#cp ${rawFolder}/tmpData/data.nii.gz ${rawFolder}/data.nii.gz
#cp ${rawFolder}/tmpData/eddyIndex.txt ${rawFolder}/eddyIndex.txt

cp ${prepFolder}/eddy/acqparamsUnwarp.txt ${prepFolder}/topup/acqparams.txt
#mv ${prepFolder}/topup/acqparamsUnwarp.txt ${prepFolder}/topup/acqparams.txt

mv ${rawFolder}/tmpData/bvals ${rawFolder}
mv ${rawFolder}/tmpData/bvecs ${rawFolder}
mv ${rawFolder}/tmpData/data.nii.gz ${rawFolder}
mv ${rawFolder}/tmpData/eddyIndex.txt ${rawFolder}
