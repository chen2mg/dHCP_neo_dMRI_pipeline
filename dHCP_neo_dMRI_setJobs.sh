...
gpuFlag=${13}                                                                     # GPU flag
testingFlag=${14}                                                                 # Testing mode (optional)

noB0s=4                                                                           # Number of B0 volumes for each PE direction used to estimate distortions with TOPUP
dimt4=$(${FSLDIR}/bin/fslval ${dataFolder}/${sessionFolder}/DWI/${dataFile} dim4) # Number of acquired volumes

echo ''
echo 'Create directory structure and write ages'
echo ============================================================================
...
echo ''
echo 'Prepare super-resolution commands (if needed)'
echo ============================================================================
if [ "${srFlag}" -eq "1" ]; then
...
if [ "${testingFlag}" -eq "1" ]; then

  echo ''
  echo 'step 1: Import files and create initial structure'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/dHCP_neo_dMRI_importFiles.sh ${dataFolder}/${sessionFolder}/DWI ${dataFile} ${subjOutFolder} ${acqProt} ${dimt4} ${noB0s}
  echo 'end of Import files and create initial structure'

  echo ''
  echo 'step 2: Run TOPUP'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/dHCP_neo_dMRI_runTopup.sh ${subjOutFolder}
  echo 'end of Run TOPUP'

  echo ''
  echo 'step 3: Run EDDY'
  read -p "Press enter to continue"
  echo ============================================================================
  if [ "${gpuFlag}" -eq "1" ]; then
    ...
    ${scriptsFolder}/dHCP_neo_dMRI_runEddy.sh ${subjOutFolder} ${slspec} ${gpuFlag}
  else
    ${scriptsFolder}/dHCP_neo_dMRI_runEddy.sh ${subjOutFolder} ${slspec} ${gpuFlag}
  fi
  echo 'end of Run EDDY'

  echo ''
  echo 'step 4: Super resolution for EDDY corrected data (if needed)'
  read -p "Press enter to continue"
  echo ============================================================================
  if [ "${srFlag}" -eq "1" ]; then
    ${scriptsFolder}/superres/prerun_superres.sh ${prepFolder}/eddy/eddy_corrected.nii.gz
    #${FSLDIR}/bin/fsl_sub -q long.q -j ${pre_srid} -l ${prepFolder}/logs -t ${prepFolder}/eddy/sr_commands.txt
    ${scriptsFolder}/superres/postrun_superres.sh ${prepFolder}/eddy/eddy_corrected.nii.gz
  else
    echo 'skip super resolution'
    post_srid=${eddyid}
  fi
  echo 'end Super resolution for EDDY corrected data (if needed)'

  echo ''
  echo 'step 5: Run EDDY post processing steps and DT fit'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/dHCP_neo_dMRI_runPostProc.sh ${subjOutFolder} ${srFlag} 1
  echo 'end of Run EDDY post processing steps and DT fit'

  echo ''
  echo 'step 6: Run BPX model 3'
  read -p "Press enter to continue"
  echo ============================================================================
  if [ "${gpuFlag}" -eq "1" ]; then
    ${scriptsFolder}/dHCP_neo_dMRI_runBPX.sh ${diffFolder}
  else
    echo 'BPX without GUP'
    ${scriptsFolder}/dHCP_neo_dMRI_runBPX.sh ${diffFolder}
  fi

  echo ''
  echo 'step 7: Extract tissue and brain masks from segmented volume'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/dHCP_neo_dMRI_getMasks.sh ${subjOutFolder} ${subjSeg}

  echo ''
  echo 'step 8: Register diffusion to structural T2 (native and template spaces)'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/dHCP_neo_dMRI_runRegistration.sh ${subjOutFolder} ${subjT2} ${anatFolder}/segmentation/wm_mask.nii.gz 1

  echo ''
  echo 'step 9: Generate PNGs for QC reports'
  read -p "Press enter to continue"
  echo ============================================================================
  ${scriptsFolder}/utils/generateFigs.sh ${subjOutFolder}
